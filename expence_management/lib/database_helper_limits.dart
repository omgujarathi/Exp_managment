...
//		var result = await db.rawQuery('SELECT * FROM $limitTable order by $colPriority ASC');
  ...
  Future<int> deleteall() async {
    var db = await this.database;
    int result = await db!.rawDelete('DELETE FROM $limitTable');
    return result;
  }
  ...
  Future<List<Map<String,dynamic>>> getlimits()async{
    ...
    // For loop to create a 'limit List' from a 'Map List'
    for (int i = 0; i < count; i++) {
      var kl=limit.fromMapObject(limitMapList[i]);
      categoryList.add({'category': kl.category,'limitamount':kl.limits_amount});
    }
